...
export function ThemesPage() {
  const state = useAsync(() => api.themes(), []);

  if (state.loading) return <SectionHeader title="Theme clusters" subtitle="Loading…" />;
  const all = state.data?.clusters ?? [];
  const themes = all.filter((t) => t.size >= 2);
  const singletons = all.filter((t) => t.size < 2);

  if (!all.length) {
    return (
      <div>
        <SectionHeader title="Theme clusters" />
        <EmptyState title="No clusters yet." hint="Run the cluster_themes job." />
      </div>
    );
  }
  ...
  return (
    <div>
      <SectionHeader
        ...
        subtitle={
          `${themes.length} themes identified across the ` +
          `${themes.reduce((s, t) => s + t.size, 0)} clustered tickets. ` +
          `${singletons.length} singleton/novel.`
        }
      />
            ...
            <ul className="mt-3 space-y-1.5 text-sm text-zinc-400">
              {singletons.map((s) => (
                <li key={s.cluster_id} className="flex items-start gap-2">
                  <span className="mt-0.5 font-mono text-[10px] text-zinc-500">{s.cluster_id}</span>
