...
import { gapRowsToCsv, type GapSheetRow } from "@/lib/gap-sheet";
...
export async function GET() {
  const rows: GapSheetRow[] = getGaps().map((gap) => ({
  ...
  return new Response(gapRowsToCsv(rows), {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="boldr-knowledge-gaps-google-sheet-${todayStamp()}.csv"`,
