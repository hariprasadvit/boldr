import { getSummary, getThemes } from "@/lib/data";
import { PERSONA_CAMPAIGNS, type PersonaCampaign } from "@/lib/campaigns";
import { EmptyState, Pill, SectionHeader, Stat } from "../components";

function personaVolume(id: string, counts: Record<string, number> | undefined): number {
  return counts?.[id] ?? 0;
}

function relatedTheme(persona: PersonaCampaign, themes: ReturnType<typeof getThemes>) {
  return themes
    .filter((theme) => Object.keys(theme.persona_breakdown ?? {}).includes(persona.id) && theme.suggested_action)
  ...
  if (!summary) {
    return (
      <div>
        ...
        <EmptyState title="No persona data found yet." hint="Run python batch_replay.py, then export the dashboard data." />
      </div>
    );
  }

  const sortedCampaigns = [...PERSONA_CAMPAIGNS].sort(
    (a, b) => personaVolume(b.id, summary.by_persona) - personaVolume(a.id, summary.by_persona),
  );
  const topPersona = sortedCampaigns[0];
  const totalAssets = PERSONA_CAMPAIGNS.reduce(
    (sum, persona) => sum + persona.angles.reduce((angleSum, angle) => angleSum + angle.assets.length, 0),
    0,
  );
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat label="Campaign angles" value={PERSONA_CAMPAIGNS.reduce((sum, p) => sum + p.angles.length, 0)} />
        ...
        <Stat
          ...
          hint={`${personaVolume(topPersona.id, summary.by_persona)} tickets`}
        />
      </div>
      ...
      <div className="mb-8 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--muted)]">
              One customer-intelligence signal is converted into a campaign angle, a newsletter, then channel-specific
              short-form assets.
            </p>
          </div>
        ...
        <div className="grid gap-3 text-xs md:grid-cols-4">
          {["Persona signal", "Campaign angle", "Newsletter source", "Social repurposing"].map((step, index) => (
      ...
      <div className="space-y-5">
        {sortedCampaigns.map((persona) => {
          ...
          const volume = personaVolume(persona.id, summary.by_persona);

          return (
            <section key={persona.id} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5">
              <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
                <div className="max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-base font-semibold text-zinc-100">{persona.name}</h2>
                    <Pill color={persona.priority === "high" ? "amber" : persona.priority === "low" ? "zinc" : "emerald"}>
                ...
                {theme && (
                  <div className="max-w-sm text-xs text-zinc-400">
                    <div className="mb-1 text-[10px] uppercase tracking-wider text-[var(--muted)]">Live theme signal</div>
              ...
              <div className="space-y-5">
                {persona.angles.map((angle, index) => (
                  <div key={angle.name} className="border-t border-[var(--border)] pt-5 first:border-t-0 first:pt-0">
                    ...
                    <div className="grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                      <div>
                        <div className="mb-1 text-[10px] uppercase tracking-wider text-[var(--muted)]">Newsletter</div>
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        {angle.assets.map((asset) => (
                          <div key={`${angle.name}-${asset.platform}`} className="rounded border border-[var(--border)] bg-zinc-950/50 p-3">
                            <div className="mb-2 flex flex-wrap items-center gap-2">
                              <Pill color={asset.platform === "Instagram" ? "rose" : "emerald"}>{asset.platform}</Pill>
